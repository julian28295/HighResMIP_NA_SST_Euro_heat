...
ERA5_variable = ['sea_surface_temperature','t2m','z300','surface_latent_heat_flux']
FOCI_variable = ['tsw_', 'temp2_','geopoth_pl_300hPa_','ahfl_']
variable = ['tos_', 'tas_']#,'zg_','hfls_']
resolution = [[''],
              ['-LL', '-MM', '-HH'],
              ['-1' , '-1-HR'],
              ['', '-HR'],
...
var_name = [['var34', 'var167','z','var147'],['tos', 'tas','zg23','hfls'],['tos', 'tas','zg10','hfls'] ,['tos', 'tas','zg','hfls'] ,['tos', 'tas','zg','hfls'] ,['tos', 'tas','zg','hfls'],['tos', 'tas','zg','hfls'],['tsw', 'temp2','geopoth','ahfl']]# 'slp_','precip_', 'ahfl_', 'ahfs_']
    ...
    for var in [0]:
        ...
        for res in range(len(resolution[mod])):
            if mod==6 and res>1: # change time axis from float to datetime in MPI-ESM by overwriting time dimension in ER with ER tos time dim
                HighRes_model [model_name[mod]+variable[1]+resolution[mod][res]]['time'] = HighRes_model [model_name[mod]+variable[0]+resolution[mod][res]]['time']
            if mod==7: # Change LSM in FOCI and use ERA5 LSM
               HighRes_model[model_name [mod]+ variable[0]+ resolution[mod][res]][var_name [mod][var]] = xr.DataArray(xr.where(HighRes_model[model_name [0]+ variable[0]+ resolution[0][0]][var_name [0][0]].sel(time='1979-01-01T06:00:00').squeeze().isnull(),HighRes_model[model_name [0]+ variable[0]+ resolution[0][0]][var_name [0][0]].sel(time='1979-01-01T06:00:00').squeeze(),HighRes_model[model_name [mod]+ variable[0]+ resolution[mod][res]][var_name [mod][var]]).transpose('time', 'lat', 'lon').data,dims=['time', 'lat', 'lon'],coords=dict(time = HighRes_model[model_name [mod]+ variable[0]+ resolution[mod][res]].time.data, lat = HighRes_model[model_name [mod]+ variable[0]+ resolution[mod][res]].lat.data, lon = HighRes_model[model_name [mod]+ variable[0]+ resolution[mod][res]].lon.data))

            if var==0:
                HighRes_model_null[model_name[mod]+variable[var]+resolution[mod][res]+'_null'] = HighRes_model[model_name[mod]+variable[var]+resolution[mod][res]][var_name [mod][var]].squeeze().fillna(HighRes_model[model_name[mod]+variable[1]+resolution[mod][res]][var_name [mod][1]]).squeeze()
            else:
        ...
        # 6 Bootstrap method
            ...
            #boots_mean_quant_025_map_lag0[model_name[mod]+variable[var]+resolution[mod][res]] = boots_mean_map[model_name[mod]+variable[var]+resolution[mod][res]].quantile(0.025,'sample_number')
            #boots_mean_quant_975_map_lag0[model_name[mod]+variable[var]+resolution[mod][res]] = boots_mean_map[model_name[mod]+variable[var]+resolution[mod][res]].quantile(0.975,'sample_number')
            boots_mean_quant_025_map_lag0[model_name[mod]+variable[var]+resolution[mod][res]] = xr.concat(boots_map[model_name[mod]+variable[var]+resolution[mod][res]], dim='sample_number').quantile(0.025,'sample_number')
            boots_mean_quant_975_map_lag0[model_name[mod]+variable[var]+resolution[mod][res]] = xr.concat(boots_map[model_name[mod]+variable[var]+resolution[mod][res]], dim='sample_number').quantile(0.975,'sample_number')
...
boots_mean_quant_025_map_lag0_ds.to_netcdf('/work/bm1398/m301111/Models/HighResMIP/boots_mean_quant_025_map_lag5_SST_T2m.nc')
boots_mean_quant_975_map_lag0_ds.to_netcdf('/work/bm1398/m301111/Models/HighResMIP/boots_mean_quant_975_map_lag5_SST_T2m.nc')
print('NetCDF Files of SST/T2m  bootstrapping composites successfully saved')
