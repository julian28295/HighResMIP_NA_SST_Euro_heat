...
for mod in range(len(model_name)):
    ...
    for var in [2]:
        ...
        # 6 Bootstrap method
            ...
            HighRes_model_detrend [model_name[mod]+variable[var]+resolution[mod][res]+'_detrend'] =  xr.DataArray(data=signal.detrend(HighRes_model_null[model_name[mod]+variable[var]+resolution[mod][res]+'_null'].squeeze(), axis=0), dims=["time","lat","lon"], coords=dict(time=HighRes_model_null[model_name[mod]+variable[var]+resolution[mod][res]+'_null'].time, lat=HighRes_model_null[model_name[mod]+variable[var]+resolution[mod][res]+'_null'].lat, lon=HighRes_model_null[model_name[mod]+variable[var]+resolution[mod][res]+'_null'].lon))
            # Deseasonalize
